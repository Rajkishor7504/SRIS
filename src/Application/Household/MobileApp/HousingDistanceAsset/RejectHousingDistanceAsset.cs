...
{
   public class RejectHousingDistanceAsset : IRequest<HousingDistanceAssetVM>
    {
    ...
    public class RejectHousingDistanceAssetHandler : IRequestHandler<RejectHousingDistanceAsset, HousingDistanceAssetVM>
    {
        ...
        public RejectHousingDistanceAssetHandler(IHouseholdService iHouseholdService, IHouseholdServiceMobile iHouseholdServiceMobile)
        {
            _iHouseholdService = iHouseholdService;
            _iHouseholdServiceMobile = iHouseholdServiceMobile;
        }

        public async Task<HousingDistanceAssetVM> Handle(RejectHousingDistanceAsset request, CancellationToken cancellationToken)
        {
            int housingretval = 0;
                ...
                else
                {
                    ...
                    if (request.housing != null)
                    {
                        ...
                        if (housingcnt > 0)
                        {
                            ...
                            entity.housinginfoid = Convert.ToInt32(request.housing.housingInfoId);
                        ...
                        if (housingcnt > 0)
                        {
                            ...
                            if (request.distance.Count > 0)
                            {
                                foreach (var item in request.distance)
                                {
                                    ...
                                    {
                                        distanceid = item.distanceid != "" ? Convert.ToInt32(item.distanceid) : 0,
                                        distanceinkm = item.distanceinkm != "" ? Convert.ToDecimal(item.distanceinkm) : 0,
                        ...
                        if (request.householdAsset.Count > 0)
                        {
                            ...
                            if (assetcnt > 0)
                            {
                                foreach (var item in request.householdAsset)
                                {
                                    ...
                                    {
                                        assetdetailid = item.assetdetailid != "" ? Convert.ToInt32(item.assetdetailid) : 0,
                                ...
                                entity2.assetid = Convert.ToInt32(request.householdAsset[0].assetdetailid);
                    ...
                    // if ((housingretval == 200 || housingretval==201) && (distanceretval == 200 || distanceretval == 201) && (assetretval == 200 || assetretval == 201))
