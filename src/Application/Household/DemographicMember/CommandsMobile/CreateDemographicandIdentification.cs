...
{
   public class CreateDemographicandIdentification : IRequest<DemographicVM>
    {
    ...
    public class CreateDemographicandIdentificationHandler : IRequestHandler<CreateDemographicandIdentification, DemographicVM>
    {
        ...
        public CreateDemographicandIdentificationHandler(IHouseholdService iHouseholdService, IWebHostEnvironment iHostingEnvironment, IApplicationDbContext context, IGrievanceService igrievanceService)
        {
        ...
        public async Task<DemographicVM> Handle(CreateDemographicandIdentification request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                //string webrootpath = _iHostingEnvironment.WebRootPath;
                ...
                //for getting settlement code by passing settlement id
                ...
                string basefolder = "Demographic";
                string BaseFolderPath = Path.Combine(webrootpath, basefolder);
                ...
                hhentity.createdby = request.createdby.ToString();
                //hhentity.hhcode =(Convert.ToInt32(request.identification.spotchecker)==1?"S":"")+ settlementcode + "" + request.identification.compoundNo.ToString() + "" + request.hhNo.ToString();
                ...
                hhentity.supervisor = request.identification.supervisor!=null ? request.identification.supervisor:"";               
                retval = await _iHouseholdService.AddRegisterHousehold(hhentity);
                if(retval==200)
                {       
                    GetRegisterHouseholdQuery objreg = new GetRegisterHouseholdQuery();
                    objreg.action = "M";
                    objreg.householdno = request.identification.hhCode != null ? request.identification.hhCode : "";
                    var hhreghhid = _iHouseholdService.GetRegisterHousehold(objreg).Result.LastOrDefault();
                    ...
                    foreach (var demographic in request.demographic)
                   {
                        ...
                        if (getmemberid > 0)//if member id exit then execute this condition
                        {
                            index = getmemberid;
                        }
                        else
                        ...
                        if (((demographic.firstname!=null) || (demographic.lastname != null)) && hhreghhid!=null)
                        {
                    ...
                    entity.relationshiptohhid = Convert.ToInt32(demographic.relationToHhId);
                     ...
                     // entity.uploadphotoname = demographic.p;
                    ...
                    if(request.demographic.Count == demoindex && retval==200)
                    {                        
                            GetDemographicMemberQuery objdemo = new GetDemographicMemberQuery();
                            objdemo.action = "VM";
                            objdemo.hhid = hhreghhid.hhid;
                            ...
                            objDemoList.hhid = hhreghhid.hhid.ToString();
